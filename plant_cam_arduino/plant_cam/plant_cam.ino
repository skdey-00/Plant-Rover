 ...
 * - Single frame capture at /capture
...
// Frame timing
const int FRAME_INTERVAL_MS = 50;  // ~20 FPS max
...
void startStreamClient(WiFiClient& client) {
    ...
    if (slot < 0) {
        sendError(client, 503, "Server busy - max clients");
        client.stop();
        Serial.println("Stream rejected: all slots busy");
        return;
    }

    Serial.printf("Stream client connected (slot %d)\n", slot);
...
// ============================================================
// Update all active stream clients
// ============================================================
